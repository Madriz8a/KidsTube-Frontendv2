document.addEventListener('DOMContentLoaded', function() {
    ...
    /**
     * Carga los datos de la playlist usando GraphQL
    ...
    async function loadPlaylistData(playlistId) {
      try {
        ...
        // Consulta GraphQL para obtener los datos de la playlist
        const graphqlQuery = `
        ...
        if (result.errors) {
          throw new Error(result.errors[0].message || 'Error al cargar los datos de la playlist');
        }
    ...
    /**
     * Carga los perfiles disponibles y marca los asignados a la playlist usando GraphQL
    ...
    async function loadProfiles(assignedProfiles) {
      try {
        ...
        // Verificar errores en la respuesta GraphQL
        if (result.errors) {
          throw new Error(result.errors[0].message || 'Error al cargar los perfiles');
        }
    ...
    /**
     * Carga los videos de la playlist usando GraphQL
    ...
    async function loadVideos(playlistId) {
      try {
        ...
        // Consulta GraphQL para obtener los videos de la playlist
        const graphqlQuery = `
        ...
        // Verificar errores en la respuesta GraphQL
        if (result.errors) {
          throw new Error(result.errors[0].message || 'Error al cargar los videos');
        }
        
        // Obtener los videos
        videos = result.data.videosByPlaylist;
    ...
    async function handleFormSubmit(e) {
      ...
      try {
        // Actualizar la playlist (se mantiene como REST porque es una operación PATCH)
     ...
     * Esta operación se mantiene como REST porque es una operación PATCH
     * @param {string} playlistId - ID de la playlist
    ...
    async function handleDeleteVideo() {
      try {
        ...
        const response = await fetch(`${API_URL}/admin/videos?id=${currentVideoToDelete}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
